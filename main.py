...
# 站点信息（预留冗余接口，用于 api 返回，默认网站网页没用到）
...
def save_data(data):
    ...
    os.replace(temp_file, DATA_FILE)  # 保证写入原子性
...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            if path == "/":
                ...
                # 返回 go.html 页面，由前端 JS 调用 API 跳转
    ...
    def send_error_response(self, code, message):
        """发送通用错误响应（JSON 格式，用于 API）"""
        self.send_response(code)
        self.send_header("Content-Type", "application/json; charset=utf-8")
        self.end_headers()
        error_data = {"code": -1, "why": message}
    ...
    def api_create(self, query):
        ...
        if not raw_link:
            return self.send_json({"code": -1, "why": "unknow"})  # 未知错误（缺少link）
        ...
        save_data(data)
        self.send_json({"code": 200, "id": sid})
...
def main():
    ...
    # 启动后台清理线程（首次立即清理一次）
    clean_expired_links()
    cleanup_thread = threading.Thread(target=periodic_cleanup, args=(3600,), daemon=True)
